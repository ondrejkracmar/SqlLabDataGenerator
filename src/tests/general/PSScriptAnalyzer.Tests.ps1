...
Param (
	...
	[string[]]
	$CommandPath = @("$global:testroot\..\SqlLabDataGenerator\functions", "$global:testroot\..\SqlLabDataGenerator\internal\functions")
)
...
Describe 'Invoking PSScriptAnalyzer against commandbase' {
	...
	Context "Analyzing <BaseName>" -ForEach $commandFiles {
		BeforeAll {
			$analysis = Invoke-ScriptAnalyzer -Path $FullName -ExcludeRule PSAvoidTrailingWhitespace, PSUseShouldProcessForStateChangingFunctions, PSUseOutputTypeCorrectly, PSUseSingularNouns
		}
