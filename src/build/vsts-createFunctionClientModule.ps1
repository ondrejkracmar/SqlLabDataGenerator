...
$parentModule = 'SqlLabDataGenerator'
if (-not $ModuleName) { $ModuleName = 'SqlLabDataGenerator.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\SqlLabDataGenerator'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\SqlLabDataGenerator.psd1"
$prereqHash = @{
