function Invoke-SldgDataGeneration {
	...
	$provider = if ($ConnectionInfo) { Get-SldgProviderInternal -ConnectionInfo $ConnectionInfo } else { $null }
	...
	$fkValues = @{}
	$script:SldgFkWarningIssued.Clear()
	...
	# Parallel config
	$useParallel = $Parallel -and -not ($Plan.Mode -eq 'Masking')
	...
	# ── Parallel generation path (Synthetic/Scenario only) ──
	...
	foreach ($tablePlan in $Plan.Tables) {
		...
		Invoke-PSFProtectedCommand -ActionString 'Generation.InsertingTable' -ActionStringValues $tablePlan.RowCount, $tablePlan.SchemaName, $tablePlan.TableName -Target $tablePlan.FullName -ScriptBlock {
			...
			$tableResult.RequestedRows = [int]$tablePlan.RowCount
			$tableResult.SkippedRows = $skippedRows
		...
		if (Test-PSFFunctionInterrupt) {
			...
			$failedResult.RequestedRows = [int]$tablePlan.RowCount
			$failedResult.SkippedRows = [int]$tablePlan.RowCount
		...
		for ($i = $tableResultCountBefore; $i -lt $tableResults.Count; $i++) {
			$tableResults[$i].DurationMs = $tableElapsedMs
		}
	}
	...
	foreach ($tableResult in $tableResults) {
		...
		$tableResult.RequestedRows = $requestedRowsForTable
		$tableResult.SkippedRows = $skippedRowsForTable
	}
	...
	if ($ValidateAfterGeneration) {
		# The plan carries everything the validators need; rebuild a SchemaModel from it so
		# validation runs against the same tables and columns that were generated.
		$validationSchema = [SqlLabDataGenerator.SchemaModel]@{
			Database     = $Plan.Database
			Tables       = @($Plan.Tables | ForEach-Object { ConvertTo-SldgTableInfo -TablePlan $_ })
			TableCount   = $Plan.TableCount
			DiscoveredAt = Get-Date
		}
		$validationResults = @(Test-SldgGeneratedData -Schema $validationSchema -ConnectionInfo $ConnectionInfo)
	...
	$result.ValidationResults = @($validationResults | Where-Object { $null -ne $_ })
	$result
	}
}
