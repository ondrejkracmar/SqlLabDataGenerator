function Test-SldgForeignKeyIntegrity {
	...
	param (
		...
		[Parameter(Mandatory)]
		[SqlLabDataGenerator.SchemaModel]$SchemaModel
	)
	...
	foreach ($table in $SchemaModel.Tables) {
		foreach ($fk in $table.ForeignKeys) {
			...
			$parentName = Get-SldgSafeSqlName -SchemaName $table.SchemaName -TableName $table.TableName -ConnectionInfo $ConnectionInfo
			$parentCol = Get-SldgSafeSqlName -ColumnName $fk.ParentColumn -ConnectionInfo $ConnectionInfo
			$refName = Get-SldgSafeSqlName -SchemaName $fk.ReferencedSchema -TableName $fk.ReferencedTable -ConnectionInfo $ConnectionInfo
			$refCol = Get-SldgSafeSqlName -ColumnName $fk.ReferencedColumn -ConnectionInfo $ConnectionInfo
