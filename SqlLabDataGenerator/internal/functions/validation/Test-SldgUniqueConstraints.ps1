function Test-SldgUniqueConstraints {
	...
	foreach ($table in $SchemaModel.Tables) {
		...
		foreach ($col in $uniqueCols) {
			...
			$safeName = Get-SldgSafeSqlName -SchemaName $table.SchemaName -TableName $table.TableName -ConnectionInfo $ConnectionInfo
			$safeCol = Get-SldgSafeSqlName -ColumnName $col.ColumnName -ConnectionInfo $ConnectionInfo
