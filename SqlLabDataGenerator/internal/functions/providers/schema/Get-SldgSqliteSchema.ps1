function Get-SldgSqliteSchema {
	...
	$tableInfos = foreach ($tableName in $tables) {
		$safeTableName = $dialect.QuoteIdentifier($tableName)
		...
		foreach ($idxName in $uniqueIndexes) {
			$safeIdxName = $dialect.QuoteIdentifier($idxName)
			$cmd = $conn.CreateCommand()
			try {
		...
		try {
			$createCmd.CommandText = "SELECT sql FROM sqlite_master WHERE type='table' AND name = $($dialect.ParameterPlaceholder('name'))"
			$nameParam = $createCmd.CreateParameter()
			$nameParam.ParameterName = $dialect.ParameterName('name')
			$nameParam.Value = $tableName
			[void]$createCmd.Parameters.Add($nameParam)
		...
		$colObjects = foreach ($col in $columns) {
			...
			# SQLite has type affinities, not types: a declared 'VARCHAR(50)' is TEXT affinity,
			# 'INTEGER PRIMARY KEY' is the rowid, and anything may be declared. Map the declared
			# name through the affinity rules first, then onto the engine's canonical vocabulary.
			$declared = ($col.type -replace '\(.*\)', '').Trim()
			$dataType = switch -Regex ($declared.ToUpperInvariant()) {
				'INT'                            { if ($declared -match '(?i)^(big|hugeint)') { 'bigint' } elseif ($declared -match '(?i)^(small|tiny)') { 'smallint' } else { 'int' } }
				'CHAR|CLOB|TEXT'                 { ConvertTo-SldgCanonicalDataType -DataType $declared }
				'BLOB'                           { 'varbinary' }
				...
				'JSON'                           { 'json' }
				default                          { if ($declared) { ConvertTo-SldgCanonicalDataType -DataType $declared } else { 'nvarchar' } }
			}
