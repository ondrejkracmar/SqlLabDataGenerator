function Get-SldgSqlServerSchema {
	...
	.DESCRIPTION
		...
		foreign keys, primary keys, unique constraints, check constraints and the view
		definitions that hint at JSON/XML columns. Filtering by schema/table is applied in
		PowerShell after retrieval. Used for every connection whose dialect reports the
		'SqlServer' schema source.
	#>
	[CmdletBinding()]
	param (
		[Parameter(Mandatory)]
	...
	# Helper to execute a read-only catalog query
	$executeQuery = {
		param([string]$Query)
		Invoke-SldgDbQuery -ConnectionInfo $ConnectionInfo -Query $Query
	}
