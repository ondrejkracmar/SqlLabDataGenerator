function Resolve-SldgForeignKeyFallback {
	...
	foreach ($parentKey in $fkByParent.Keys) {
		...
		$safeRef = $dialect.QualifiedName($firstFk.ReferencedSchema, $firstFk.ReferencedTable)
		try {
			$safeCols = @($parentFks | ForEach-Object { $dialect.QuoteIdentifier($_.ReferencedColumn) } | Select-Object -Unique)
			$cmd = $ConnectionInfo.DbConnection.CreateCommand()
			if ($Transaction) { $cmd.Transaction = $Transaction }
			$cmd.CommandText = $dialect.SelectLimited(($safeCols -join ', '), $safeRef, $FkQueryLimit, $true, ($safeCols -join ', '))
			...
			foreach ($fk in $parentFks) {
				...
				if ($colLists[$fk.ReferencedColumn].Count -gt 0) {
					...
					$fallbackStats.Add([SqlLabDataGenerator.ForeignKeyFallback]@{
						ReferenceKey = $refKey
