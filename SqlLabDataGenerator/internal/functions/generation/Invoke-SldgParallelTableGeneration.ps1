					...
					foreach ($fk in $tablePlan.ForeignKeys) {
						...
						if (-not $FkValues.ContainsKey($refKey) -or $FkValues[$refKey].Count -eq 0) {
							try {
								...
								$safeRef = $dialect.QualifiedName($fk.ReferencedSchema, $fk.ReferencedTable)
								$safeCol = $dialect.QuoteIdentifier($fk.ReferencedColumn)
								$cmd = $ConnectionInfo.DbConnection.CreateCommand()
								try {
									if ($Transaction) { $cmd.Transaction = $Transaction }
									$cmd.CommandText = $dialect.SelectLimited($safeCol, $safeRef, $fkQueryLimit, $true)
								...
								if ($vals.Count -gt 0) {
									...
									$fkFallbackStats.Add([SqlLabDataGenerator.ForeignKeyFallback]@{
										ReferenceKey = $refKey
