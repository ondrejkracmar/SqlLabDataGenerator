function New-SldgGeneratedValue {
	...
	if ($Column.ForeignKey -and $ForeignKeyValues) {
		...
		# FK column but no parent values available — warn ONCE per column and return sentinel $null
		# so the caller (New-SldgRowSet) can handle the missing FK with its own fallback. The same
		# situation repeats for every row of the table; one warning carries the information.
		if ($script:SldgFkWarningIssued.Add("$($Column.ColumnName)|$refKey")) {
