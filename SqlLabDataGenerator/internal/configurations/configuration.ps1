...
# Foreign-key columns without parent values warn once per column per generation run, not once
# per row. Invoke-SldgDataGeneration clears this at the start of every run.
$script:SldgFkWarningIssued = [System.Collections.Generic.HashSet[string]]::new()
