# Provider names - whatever PSSqlRepository has loaded (built-in SqlServer/Sqlite plus installed extensions)
Register-PSFTeppScriptblock -Name "SqlLabDataGenerator.Provider" -ScriptBlock {
	if (Get-Command -Name Get-PSSqlRepositoryProvider -ErrorAction SilentlyContinue) {
		Get-PSSqlRepositoryProvider | ForEach-Object Name | Sort-Object
	}
