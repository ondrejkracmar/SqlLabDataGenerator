...
@{
	...
	'Generation.MaskingNoColumns'            = 'Table {0} has no PII or rule-driven columns to mask - rows left unchanged.'
	'Generation.MaskingNoKey'                = 'Table {0} has no primary key; masked rows are written back by delete + re-insert, which cannot preserve rows referenced by other tables.'
	'Write.RowsUpdated'                      = 'Updated {0} rows in {1}'
	...
	'Write.BatchFallback'                    = "Batch insert into {0} ({1} rows) was rejected ({2}); retrying the rows one by one."
